        ...
        *:after {
        ...
        <script>
            ...
            eventSource.onmessage = function(e) {
                ...
                switch(data.channel) {
                    case "facebook":
                        icon = "https://www.flaticon.com/premium-icon/icons/svg/2335/2335286.svg";
                        break;
                    case "twitch":
                        icon = "https://www.flaticon.com/premium-icon/icons/svg/2335/2335303.svg";
                        break;
                }
                document.getElementById('msger-chat').innerHTML += '<div class="msg left-msg">'
                    + '<div class="msg-img" style="background-image: url(' + icon + ')"></div>'
...
def facebookLive():
    """"輸出 Facebook 的聊天內容。"""
    url = "https://streaming-graph.facebook.com/" + config.facebook.live_video_id + "/live_comments?access_token=" + config.facebook.access_token + "&comment_rate=one_per_two_seconds&fields=from{name,id},message"
    response = with_urllib3(url)
    client = sseclient.SSEClient(response)
    for event in client.events():
        data = json.loads(event.data)
        print(f"[Facebook] {data['from']['name']}: {data['message']}")
        publisher.publish(json.dumps({"channel": "facebook", "author": data['from']['name'], "message": data['message'], "time": time.strftime("%H:%M:%S")}))
        voice(data['message'])
...
def voice(message: str):
    ...
    speech_config = speechsdk.SpeechConfig(subscription=speech_key, region=service_region, speech_recognition_language=config.speech.speak.voice.name)
